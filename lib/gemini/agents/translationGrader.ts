...
- "wrong": a real error — either a genuinely different word with a different meaning (see THIRD CRITICAL RULE), or a garbled spelling of the intended word, or a grammar mistake. Set "note" to a specific reason, correctly framed as either a wrong word (state both meanings) or a spelling slip (state the intended word), e.g. "wrong verb — 'trae' (brings) not 'llevar' (to carry away)", "feminine adjectives end in -a".
  If the learner's word isn't a real word, work out what real Spanish word it most likely reaches for before writing the note — don't just call it "not a Spanish word". Two cases:
  ...
  - It instead resembles a DIFFERENT real, common Spanish word (e.g. "tenir" closely resembles "tener" (to have), not the intended word "tomar" (to have/drink) — different verbs). Name that real word specifically, then judge whether IT would also work naturally in this sentence: if yes, say so as a workable alternative even though it's not the word being practiced; if no, explain concretely why it doesn't fit here (what it actually means vs. what's needed) before giving the correct word. Either way, name the real word you think they meant rather than leaving the note at "not a word".
  Set "minorMistake" to true for a spelling slip or a recognizable near-miss (either case above) — anything a native speaker would likely still understand despite being non-standard. Set it to false for a genuinely different/wrong word, a grammar mistake, or anything that would likely confuse a native speaker.

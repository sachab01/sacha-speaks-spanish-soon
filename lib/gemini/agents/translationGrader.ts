...
- "wrong": a real error — either a genuinely different word with a different meaning (see THIRD CRITICAL RULE), or a garbled spelling/non-word, or a grammar mistake. First decide whether the word the learner actually wrote is itself a real Spanish word:
  - If it IS a real Spanish word (just the wrong one for here), "note" must give ITS actual Spanish meaning/translation against what's needed, e.g. "wrong verb — 'trae' (brings) not 'llevar' (to carry away)", "feminine adjectives end in -a". Never bring Italian/French into this case — the word is real Spanish, not a foreign one.
  - If it is NOT a real Spanish word, NEVER leave the note at just "'X' isn't a Spanish word" — always give a plausible account of where it came from. Work out which of these fits:
    - It's a near-miss of the vocab word itself (e.g. an anglicized guess, a half-remembered conjugation) — say so, note it would likely still be understood, alongside the standard word to use instead.
    - It resembles a DIFFERENT real, common Spanish word instead (e.g. "tenir" closely resembles "tener" (to have), not the intended word "tomar" (to have/drink) — different verbs; "per" closely resembles "por", not the intended "para" — the classic por/para mix-up). Name that real word specifically, then judge whether IT would also work naturally in this sentence: if yes, say so as a workable alternative even though it's not the word being practiced; if no, explain concretely why it doesn't fit here (what it actually means vs. what's needed) before giving the correct word.
    - It's a recognizable word from a different language this learner mixes up with Spanish — check Italian first, then French — rather than an attempted Spanish word at all (e.g. "per" is the Italian preposition for "for"/"by"/"through", as in "per favore" — not any Spanish word). State what it means in that likely source language, and that Spanish doesn't use it that way here, before giving the correct word.
  Whichever case applies, the note must tell the learner something about the word THEY wrote, not just restate the correct one.
  Set "minorMistake" to true for a spelling slip or a recognizable near-miss (any case above) — anything a native speaker would likely still understand despite being non-standard. Set it to false for a genuinely different/wrong word, a grammar mistake, or anything that would likely confuse a native speaker.
